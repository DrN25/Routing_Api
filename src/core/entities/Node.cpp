...
Node::Node(int64_t id , const Coordinate& coordinate, const std::unordered_map<std::string, std::string>& tags)
        : id(id), coordinate(coordinate), tags(tags) {}

void Node::addTag(const std::string& key, const std::string& value) {
    tags[key] = value;
}
